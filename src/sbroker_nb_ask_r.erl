...
%% @doc Lookup the pid of a process calling `sbroker:ask/1' on sbroker
%% `Broker'. Returns `Pid' if `sbroker:nb_ask_r/1' returns
%% `{go, Ref, Pid, SojournTime}' and `undefined' on `{retry, 0}'.
-spec whereis_name(Broker) -> Pid | undefined when
      Broker :: sbroker:broker(),
      Pid :: pid().
whereis_name(Broker) ->
    case sbroker:nb_ask_r(Broker) of
        {go, _, Pid, _} ->
            Pid;
        {retry, _} ->
            undefined
...
%%
%% This functions exits if `sbroker:nb_ask_r/1' returns `{retry, 0}'.
