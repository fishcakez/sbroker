...
init_it(Starter, Parent, Name, Mod, Args, Opts) ->
    ...
    try Mod:init(Args) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            Reason2 = sbroker_handlers:exit_reason({Class, Reason, ?GET_STACK(Stacktrace)}),
...
update_meter(Now, open, #time{send=Send} = Time, Q, V,
    ...
    try VMod:open_time(V) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            valve_exception(Class, Reason, open, Time, Q, V, Config, ?GET_STACK(Stacktrace))
    end;
update_meter(Now, closed, #time{send=Send, empty=Empty} = Time, Q, V,
    ...
    try QMod:send_time(Q) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            queue_exception(Class, Reason, closed, Time, Q, V, Config, ?GET_STACK(Stacktrace))
    end.
...
closed({continue, From, Ref}, #time{send=Send, now=Now} = Time, Q, V, _,
    ...
    try VMod:handle_continue(Ref, Send, V) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            valve_exception(Class, Reason, closed, Time, Q, V, Config, ?GET_STACK(Stacktrace))
    end;
...
common({update, From, Value}, State, #time{send=Send} = Time, Q, V, _,
    ...
    try VMod:handle_update(Value, Send, V) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            valve_exception(Class, Reason, State, Time, Q, V, Config, ?GET_STACK(Stacktrace))
    end;
...
common({len, From, _}, State, Time, Q, V, _,
    ...
    try QMod:len(Q) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            queue_exception(Class, Reason, State, Time, Q, V, Config, ?GET_STACK(Stacktrace))
    end;
...
config_change(#config{mod=Mod, args=Args}) ->
    try Mod:init(Args) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            {error, {Class, Reason, ?GET_STACK(Stacktrace)}}
    end.
...
opening_out(#time{now=Now, send=Send} = Time, Q, V, VNext,
    ...
    try QMod:handle_out(Now, Q) of
        ...
        ?EXCEPTION(Class, Reason, Stacktrace) ->
            queue_exception(Class, Reason, open, Time, Q, V, Config, ?GET_STACK(Stacktrace))
    end.
...
queue_exception(Class, Reason, State, Time, Q, V,
                #config{queue_mod=QMod, valve_mod=VMod} = Config, Stacktrace) ->
